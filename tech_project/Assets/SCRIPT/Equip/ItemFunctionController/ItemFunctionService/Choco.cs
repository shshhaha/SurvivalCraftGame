...
public class Choco : MonoBehaviour
{
    private PlayerDTO pdto; // VO 접근
        public static Choco instance;
        private int ChocoHP = 5; // 체력 회복량 (임시)    
        ...
        public static Choco Instance
        {
            get
            {
                if (instance == null)
                {
                    instance = GameObject.FindObjectOfType<Choco>();
                }
                return instance;
            }
        }
        ...
        public void ChocoLogic()
        {
            ...
            if (pdto != null)
            {
                pdto.setCurrentHp((int)pdto.getCurrentHp() + ChocoHP);
            }
            else
            {
