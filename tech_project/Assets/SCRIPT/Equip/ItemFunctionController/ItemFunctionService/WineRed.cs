...
namespace UseItem
{
public class WineRed : MonoBehaviour
{
    private PlayerDTO pdto; // VO 접근
        public static WineRed instance;
        //private int mushRoomHP = 10; // 체력 회복량 (임시)    
        private int WineRedST = 10; // 스태미나 회복량 (임시)
        ...
        public static WineRed Instance
        {
            get
            {
                if (instance == null)
                {
                    instance = GameObject.FindObjectOfType<WineRed>();
                }
                return instance;
            }
        }

        public void WineRedLogic()
        {
            if (pdto == null)
            {
            ...
            if (pdto != null)
            {
                int newStamina = (int)pdto.getHunger() + WineRedST;
                if (newStamina > 100) {
                    newStamina = 100;
                }
